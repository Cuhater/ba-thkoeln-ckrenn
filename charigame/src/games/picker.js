(function ($, $window, $document, undefined) {
	...
	$.extend(trianglePicker.prototype,
		{
			...
			handlePosition: function (set, value) {
				...
				if (value !== undefined) {
					var updates = {
						top: value.top - this.handleDimensions().height / 2,
						left: value.left - this.handleDimensions().width / 2
					}
			...
			pickerPoints: function (set) {
				...
				this._pickerPoints = {
					bottomLeft: {
						x: this.options.polygon.line.width,
						y: this.svgDimensions().height - this.options.polygon.line.width / 2
					},
					topMiddle: {x: this.svgDimensions().width / 2, y: this.options.polygon.line.width},
					bottomRight: {
						x: this.svgDimensions().width - this.options.polygon.line.width,
						y: this.svgDimensions().height - this.options.polygon.line.width / 2
					},
	...
	function setUpPicker(plugin, options) {
		...
		plugin.handle = $('<div id="triangle-picker-handle" class="' + pickerHandleClass + '"></div>')
	...
	function makeInput(opts, type) {
		return $('<input type="number"' +
			...
			'" name="' + (typeof opts.id === 'string' ? opts.id : '') +
	...
	function setupPickerLabels($wrapper, options) {
		...
		$wrapper.prepend($('<div class="triangle-picker-top-label-wrapper"><label class="topMiddleLabel triangle-picker-label max-sm:text-base">' + getInputName('topMiddle', options) + '</label></div>'))
		var bottomContainer = $('<div class="triangle-picker-bottom-label-wrapper"></div>')
		bottomContainer.append($('<label class="bottomLeft triangle-picker-label max-sm:text-base">' + getInputName('bottomLeft', options) + '</label>'))
		bottomContainer.append($('<label class="bottomRight triangle-picker-label max-sm:text-base">' + getInputName('bottomRight', options) + '</label>'))
	...
	function getCombinedLinePoints(type, pickerPoints, options) {
		...
		if (type === 'bottomLeft')
			overwrite = combinePoints(mid, {
				x: pickerPoints.bottomLeft.x + centerLineWidth,
				y: pickerPoints.bottomLeft.y - centerLineWidth / 2
			})
			...
			overwrite = combinePoints(mid, {
				x: pickerPoints.bottomRight.x - centerLineWidth,
				y: pickerPoints.bottomRight.y - centerLineWidth / 2
			})
		return overwrite
	}
	...
	function moveToPolygonCenter($handle, handleDimensions, svgDimensions) {
		...
		$handle.css({
			top: heightToCenter - handleDimensions.height / 2,
			left: svgDimensions.width / 2 - handleDimensions.width / 2
		})
	}
	...
	function pointInTriangle(point, triVec0, triVec1, triVec2) {   // using barycentric coordinates
	...
	function setUpHandleStyles($handle, options) {
		...
		if (options.handle.backgroundImage !== null) $handle.css('background-image', 'url(' + options.handle.backgroundImage + ')');
